 ...
 * 2023/2/16 18:49
 */
public class ApplicantLoginInterceptor implements HandlerInterceptor {
    /***
     * 在请求处理之前进行调用(Controller方法调用之前)
     */
    @Override
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        System.out.println("执行了登录拦截器的preHandle方法");
        try {
            HttpSession session = request.getSession();
            Object applicant = session.getAttribute("applicant");
            if (applicant != null) {
                return true;
            }
        } catch (Exception e) {
